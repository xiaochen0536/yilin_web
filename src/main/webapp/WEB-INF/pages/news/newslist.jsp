...
<body>
	<form method="post" action="${basePath }News/login" id="form1">
		...
		<div>
            ...
            <div id="NoLong" style="float:left;width:277px;">
                ...
                <a href="login2.jsp"  class="login">登录</a><a href="register2.jsp" 
                    ...
                    <input name="iii"  type="text" id="txt_keyWork" class="seachTxt" placeholder="搜索" />
			...
			<div id="nav">
				<div class="nav">
					<ul>
						...
						<li><a href="${basePath }News/login"><b class="txt">新闻</b><span
			...
			<div class="content">
				<div class="newstop">
					...
					<div class="newstopright">
						<div class="sousuozi">
							<input name="ntitle" value="${NewsList.ntitle }" type="text" />
						</div>
						<div class="sousuo">
						...
						<div class="booknewslist_left">
							...
							<div class="newslist_page">
								...
								<div id="ContentPlaceHolder1_AspNetPager1" class="anpager">
									<a
										href="${basePath }News/login?ntitle=${NewsList.ntitle }&i=1">首页</a><a 
										href="${basePath }News/login?ntitle=${NewsList.ntitle }&i=${NewsList.i -1}">上一页</a>
										<c:forEach varStatus="status" begin="1" end="${NewsList.mo }">
											<c:choose>
											   <c:when test="${NewsList.i ==status.index }">    	<!-- 如果 -->
											   		<span><b><font color="red">${status.index }</font></b></span>
											   </c:when>
											   
											   <c:otherwise>  		<!-- 否则 -->
											   		<a href="${basePath }News/login?ntitle=${NewsList.ntitle }&i=${status.index}">${status.index}</a>
											   </c:otherwise>
											 </c:choose>
										</c:forEach>
										<a href="${basePath }News/login?ntitle=${NewsList.ntitle }&i=${NewsList.i +1}">下一页</a>
										<a href="${basePath }News/login?ntitle=${NewsList.ntitle }&i=${NewsList.mo}">末页</a>
								</div>
